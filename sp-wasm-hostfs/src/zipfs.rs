    ...
    fn new(path : impl Into<path::PathBuf> + 'static) -> io::Result<Self> {
        ...
        for i in 0..archive.len() {
            ...
            if file.is_file() {
                ...
                files.insert(path, ZipFsNodeRef::Dir { idx: i, nodes: HashMap::new() });
            }
        }
        let root : Rc<Path>= PathBuf::from("").into();
        files.insert(root.clone(), ZipFsNodeRef::Root { nodes: HashMap::new()});
        paths.sort();
        ...
        let inner = Arc::new(Mutex::new(ZipFsInner {
            path,
            archive
        }));
...
impl ZipFsNode {
    ...
    fn find_node(&self, name : &str) -> io::Result<Option<&Arc<ZipFsNodeRef>>> {
        let nodes = match self.node_ref.as_ref() {
            ZipFsNodeRef::Dir { nodes, ..} =>
                nodes,
            ZipFsNodeRef::Root {  nodes} =>
                nodes,
...
mod test {
    use super::ZipFs;
    use crate::vfsops::{VfsVolume, INode};
    ...
    fn test() -> failure::Fallible<()> {
        ...
        while !stack.is_empty() {
            ...
            eprintln!(":: {}", path);
            match n_type {
                NodeType::Dir => for child in inode.read_dir().unwrap() {
                    stack.push((format!("{}/{}", path, child), inode.lookup(&child)?.unwrap()))
                },
                _ => {
                    n_files+=1;
                    ()
